/**
 * public/js/todos-page.js
 * ─────────────────────────────────────────────────────────────────────────────
 * Módulo da página dedicada de Todos os Anúncios Coletados.
 * Exibe a totalidade dos anúncios do banco, colorindo-os de acordo com
 * sua classificação (Blacklist = vermelho, Safelist = verde).
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
function createEmptyState() {
  ...
  div.innerHTML = `
    ...
    <div class="empty-desc">Anúncios aparecerão aqui após as pesquisas dos bots.</div>
  `;
  return div;
}

// ── Carregar Todos os Anúncios ──────────────────────────────────────────────

async function loadAllAds() {
  const offset = todosState.page * todosState.perPage;
  const params = new URLSearchParams({
    limit: todosState.perPage,
    offset,
    orderBy: todosState.orderBy
  });

  if (todosState.filterKeyword) params.set('keyword', todosState.filterKeyword);
  if (todosState.filterDomain) params.set('domain', todosState.filterDomain);
  ...
  try {
    ...
    todosState.ads = data.ads || [];
    todosState.total = data.total || 0;

    renderAdsTable();
    updatePagination();
  } catch (err) {
    console.error('[Todos] Erro ao carregar anúncios:', err);
  }
}
...
function updatePagination() {
  ...
  const start = todosState.page * todosState.perPage + 1;
  const end = Math.min(start + todosState.perPage - 1, todosState.total);
  const totalPages = Math.ceil(todosState.total / todosState.perPage);

  info.textContent = `${start}–${end} de ${todosState.total} anúncio(s) · Página ${todosState.page + 1}/${totalPages}`;
  prevBtn.disabled = todosState.page <= 0;
  nextBtn.disabled = end >= todosState.total;
}
...
function setupEventListeners() {
  ...
  btnApply?.addEventListener('click', () => {
    todosState.filterKeyword = filterKeywordInput?.value.trim() || '';
    todosState.filterDomain = filterDomainInput?.value.trim() || '';
    todosState.orderBy = orderBySelect?.value || 'recent';
    todosState.page = 0;
    loadAllAds();
  });
  ...
  document.getElementById('btnAdsNextPage')?.addEventListener('click', () => {
    const maxPage = Math.ceil(todosState.total / todosState.perPage) - 1;
    if (todosState.page < maxPage) {
      todosState.page++;
      loadAllAds();
    }
  });
